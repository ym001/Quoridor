...
/**
    Obtenir l'action décidé par l'arbitre.
...
*/
	boolean coupValide(String str,Joueur ja,Plateau p){
		...
		if(action[0].equals("p")){
			if(action.length==3){
				try{
					int arriveX=Integer.parseInt(action[1]);
					int arriveY=Integer.parseInt(action[2]);
					int departX=ja.pion().abscisse();
					int departY=ja.pion().ordonnee();
					int deplacementX=arriveX-departX;
					int deplacementY=arriveY-departY;
					/*int signX=0;
					int signY=0;
					if(deplacementX>0){signX=1;}
					if(deplacementX<0){signX=-1;}
					if(deplacementY>0){signY=1;}
					if(deplacementY<0){signY=-1;}
					*/
					if((arriveX<1)||(arriveY<1)||(arriveX>17)||(arriveY>17)){valider=false;message="\nLe pion doit rester à l'intérieur du plateau.\n";}
					else if((arriveX%2==0)||(arriveY%2==0)){valider=false;message="\nLes coordonnées du pion doivent être impaires.\n";}
					else if((Math.abs(deplacementX)>4)||(Math.abs(deplacementY)>4)){
						valider=false;message="\nUn pion ne peut se déplacer que d'une case sans sauter de pion.\n";
						}
					else if((Math.abs(deplacementX)==4)&&(Math.abs(deplacementY)==0)){
						if(!p.testCasePion(departX+deplacementX/2,departY)){
							valider=false;message="\nUn pion peut se déplacer de deux cases uniquement si la case intermédiare est occupée par un pion.\n";}
						else if(p.testCaseMur(departX+deplacementX/2+deplacementX/4,departY)){
							valider=false;message="\nUn pion ne peut pas sauter un pion et un mur.\n";
							}
						}
					else if((Math.abs(deplacementX)==0)&&(Math.abs(deplacementY)==4)){
						if(!p.testCasePion(departX,departY+deplacementY/2)){
							valider=false;message="\n\nUn pion peut se déplacer de deux cases uniquement si la case intermédiare est occupée par un pion.\n";}
						else if(p.testCaseMur(departX,departY+deplacementY/2+deplacementY/4)){
							valider=false;message="\nUn pion ne peut pas sauter un pion et un mur.\n";
							}
						}
					else if((Math.abs(deplacementX)==2)&&(Math.abs(deplacementY)==2)){
						valider=false;message="\nUn pion ne peut se déplacer librement en diagonnale.\n";
		//test niveau3 à completer...
						//if((p.testCaseMur(departX+deplacementX/2,departY+deplacementY+deplacementY/2))||(p.testCaseMur(departX+deplacementX+deplacementX/2,departY+deplacementY/2))){valider=false;message="\nUn pion ne peut pas sauter un mur.\n";}
						if(p.testCasePion(departX,arriveY)&&p.testCaseMur(departX,arriveY+deplacementY/2)){
							if(p.testCaseMur(departX+deplacementX/2,arriveY)){valider=false;message="Vous ne pouvez pas sauter ce pion.\n";}
							else{valider=true;message="Vous pouvez sauter ce pion.\n";
							
							}
							}
						if(p.testCasePion(arriveX,departY)&&p.testCaseMur(arriveX+deplacementX/2,departY)){
							if(p.testCaseMur(departX+deplacementX/2,arriveY)){valider=false;message="Vous ne pouvez pas sauter ce pion.\n";}
							else{valider=true;message="Vous pouvez sauter ce pion.\n";}							}
						}
					else if(p.testCaseMur(departX+deplacementX/2,departY+deplacementY/2)){valider=false;message="\nUn pion ne peut pas se déplacer à travers un mur.\n";}
					else if(p.testCasePion(arriveX,arriveY)){valider=false;message="\nLa case est occupée par un pion.\n";}
					
					else{valider=true;
						if(((arriveY==1)&&(p.occupationCase(departX,departY)==2))||((arriveY==17)&&(p.occupationCase(departX,departY)==1))){
							message="\nC'est gagné "+ja.nom()+".\n";
							jeu=false;
							}
						}
		...
		if(action[0].equals("q")){valider=true;jeu=false;}
		if(action[0].equals("")){valider=false;message="\nIl faut jouer.\n";;}
...
*/
	void regleDuJeu(){
		...
		message=message+"\nPour sortir du jeu utiliser la lettre q.\n";
